...
const injection = `<script>window.MCP_API_URL = '${apiUrl}'; window.FRONTEND_API_KEY = '${frontendApiKey}'; window.AGENT_BASE_URL = '${agentBaseUrl}';</script>`;
html = html.replace('</head>', injection + '\n</head>');

fs.writeFileSync('public/index.html', html);
console.log(`Injected MCP_API_URL = ${apiUrl}`);
console.log(`Injected FRONTEND_API_KEY = ${frontendApiKey ? '(set)' : '(empty)'}`);
console.log(`Injected AGENT_BASE_URL = ${agentBaseUrl}`);
