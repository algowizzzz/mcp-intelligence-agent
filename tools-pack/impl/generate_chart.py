"""generate_chart — interactive Plotly chart generation."""
import os
...
def _resolve_charts_dir(ctx: Dict[str, str]) -> str:
    """Return the per-user charts directory: {my_data_path}/charts/."""
    my_data = (ctx.get('my_data_path') or '').strip().rstrip('/')
    if my_data:
        return my_data + '/charts'
    raise RuntimeError('Cannot resolve charts directory: no my_data_path in worker context')


def _make_layout(title, x_label, y_label, theme_key, width, height):
    t = _THEMES.get(theme_key, _THEMES['riskgpt'])
    return {
        'title': {'text': title, 'font': {'family': 'Arial', 'size': 14, 'color': t['title']}, 'x': 0.02},
        'paper_bgcolor': t['bg'],
...
def _build_traces(chart_type, rows, x, y, color, params, theme_key):
    ...
    if chart_type in ('bar', 'bar_horizontal'):
        ...
        for i, yc in enumerate(y_cols):
            ...
            fillcolor = base_c.replace(')', ',0.2)').replace('rgb', 'rgba') if base_c.startswith('rgb') else base_c + '33'
            traces.append(go.Scatter(
                x=_col(rows, x), y=_col(rows, yc), name=yc,
                mode='lines', fill=fill,
                line={'color': base_c, 'shape': line_shape},
                fillcolor=fillcolor,
            ))
        ...
        if color:
            ...
            for i, g in enumerate(groups):
                ...
                traces.append(go.Scatter(
                    ...
                    mode='markers',
                    marker={'size': marker_size, 'color': colors[i % len(colors)]},
                ))
        else:
            traces.append(go.Scatter(
                x=_col(rows, x), y=_col(rows, yc), name=yc,
                mode='markers',
                marker={'size': marker_size, 'color': colors[0]},
            ))
...
class GenerateChart(BaseMCPTool):
    """Generate an interactive Plotly chart from row-dict data."""
    ...
    def get_input_schema(self) -> Dict:
        return self.config.get('inputSchema', {
            ...
            'properties': {
                ...
                'y':            {'description': 'Column name or list for y-axis / values'},
                'color':        {'type': 'string', 'description': 'Column for colour grouping'},
                'title':        {'type': 'string'},
                'x_label':      {'type': 'string'},
                'y_label':      {'type': 'string'},
        ...
        if output_format in ('png', 'both'):
            try:
                ...
                if not fname.endswith('.png'):
                    fname += '.png'
                charts_dir = _resolve_charts_dir(ctx)
